...
int main()
{
    ...
    sched.major_frame = hyper_period(scheduler)*DOMN_RUNTIME;
    ...
    sched.num_sched_entries = list_length(head_2);
    ...
    while(head_2 != NULL)
    {
      ...
      // sched.sched_entries[k].runtime = (head_2->ts);
       sched.sched_entries[k].runtime = DOMN_RUNTIME;
    ...
    for(int z=0;z<k;z++)
    {
        ...
        printf("Dom_Handle:%X\n",*sched.sched_entries[z].dom_handle);
        printf("Domain Runtime:%d\n",sched.sched_entries[z].runtime);
    }
  ...
  printf("Status of xc_arinc_get:%d\n",result_get);
    return 0;
}
