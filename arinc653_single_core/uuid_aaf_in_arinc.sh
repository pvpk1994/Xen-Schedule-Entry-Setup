...
if [ $(whoami) != "root" ]; then
	...
	if [ $? -eq 0 ]; then
		...
		gcc $(pwd)/aaf_prefinal.c -lxenctrl -lm -luuid -o arinc_sample -L/usr/local/lib 2>/dev/null
		#gcc $(pwd)/arinc_sample.c -lxenctrl -lm -luuid -o arinc_sample -L/usr/local/lib 2>/dev/null
		if [ $? -eq 0 ]; then
