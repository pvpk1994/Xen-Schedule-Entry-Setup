...
if [ $(whoami) != "root" ]; then
	...
	#cat list_info.txt | awk '{ for(counter = 0; counter < 3; counter++) {if ($10=="arinc_pool") {print $7}}}' >$UUID_INFO
	...
	if [ $? -eq 0 ]; then
		...
		gcc $(pwd)/arinc_prefinal.c -luuid -lxenctrl -o arinc653 -lm  -L/usr/local/lib 2>/dev/null
		if [ $? -eq 0 ]; then
