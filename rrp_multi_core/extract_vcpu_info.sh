...
UUID_INFO=$DEF_DIR/pool_uuid/uuid_info.txt
VCPU_INFO=$DEF_DIR/pool_uuid/vcpu_info.csv

if [ $(whoami) != "root" ]; then
	echo "only root access!!"
	exit 1
else
	xl list -c -v > $DEF_DIR/pool_uuid/list_info.txt
	NUM_DOMS=$(cat $DEF_DIR/pool_uuid/list_info.txt | awk 'END{print NR}')
	...
	for i in $(seq 1 $NUM_DOMS); do
		cat $DEF_DIR/pool_uuid/list_info.txt | awk '{if($10=="aaf_pool") {print $7}}' >$UUID_INFO
	done
	...
	dom_id=($(cat $DEF_DIR/pool_uuid/list_info.txt | awk '{if($10=="aaf_pool") {print $2,$7}}'))
	...
	done < $DEF_DIR/pool_uuid/list_info.txt
	...
	echo "Name,Domain ID,Domain UUID,VCPU,CPU" >> $DEF_DIR/pool_uuid/dom_vcpu_cpu.csv
	while IFS=" " read -r first second third fourth fifth sixth seventh; do
		...
		if [[ " ${dom_id[*]} " =~ " ${second} " ]]; then
			echo "$first,$second,${map[${second}]},$third,$fourth" >> $DEF_DIR/pool_uuid/dom_vcpu_cpu.csv;
