...
/* ********** UPDATE *************
 * Runnable Version: Archiving this version as it does not support valid- Xen Supported Domain ID format
    ...
    for(int i=0;i< count;i++)
    {
       ...
       // printf("SIZE:%d\n",hp_now);
        ...
        for(int k=0;k <arr[i];k++)
        {
         ...
         printf("Assigning timeslice %d to Domain %d\n",get_i(Node,index_now),sched[i].id);
    ...
    for(int i=0; i<RRP_PCPUS; i++){
       ...
       int count = 0; // Make sure count is always less than SIZE
        ...
        if(count != 0) {
            ...
            sched_aaf.cpu_id = pc[rrp_cpus[i]].cpu_id;
            sched_aaf.hyperperiod = hyper_period_MulZ(RRP_MulZ, count)*TIME_SLICE_LENGTH;
            sched_aaf.num_schedule_entries = list_length(head2);
            ...
            {
                // char buf[30];
                ret_val = uuid_parse(head2->id, sched_aaf.schedule[k].dom_handle);
            ...
            // SET-ZONE
            set_result = xc_sched_aaf_schedule_set(xci, cpu_pool_id, &sched_aaf);
            printf("Schedule Set for LT PCPU: %d is: %d\n", i, set_result);

            // GET-ZONE
            get_result = xc_sched_aaf_schedule_get(xci, cpu_pool_id, &sched_aaf);
